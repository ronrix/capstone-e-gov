<template>
    ...
    <div class="fixed top-0 left-0 right-0 bottom-0 bg-black/20 flex items-center justify-center z-30">
        <form @submit.prevent="onSubmit"
            class="backdrop-blur-xl bg-white/70 h-auto sm:w-[500px] p-5 rounded-lg flex flex-col border overflow-y-scroll scrollbar"
            ...
            <p class="text-xs text-gray-600 whitespace-pre-wrap mb-3">
                by creating new service, this will be visible on services tab where you can add requirements and processes
            </p>
            ...
            <label class="flex flex-col flex-1">
                <span class="text-xs sm:text-sm font-bold capitalize mb-2">intended for:</span>
                <input v-model="formData.intendedFor" name="intendedFor" type="text" placeholder="Type the here..."
                    class="rounded-md mb-5 p-2 focus:outline-blue-600 text-xs sm:text-sm">
                <p v-if="v$.intendedFor.$error && isError" class="text-xs text-red-400 mb-2"> {{
                    v$.intendedFor.$errors[0].$message }}
                </p>
            </label>
            <label class="flex flex-col flex-1">
                <span class="text-xs sm:text-sm font-bold capitalize mb-2">service type</span>
...
const rules = computed(() => ({
    ...
    intendedFor: { required: helpers.withMessage("The field 'intended for' is required", required) },
    serviceType: { required: helpers.withMessage("The field 'service type' is required", required) },
}));
