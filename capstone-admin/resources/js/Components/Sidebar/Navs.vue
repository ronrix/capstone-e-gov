<template>
  <nav class="mt-10 flex flex-col items-start justify-start">
    <!-- dashboard -->
    <div class="w-full">
      <RouterLink activeClass="bg-blue-600 text-white" to="/dashboard"
    ...
    <SubNavs v-show="isWholeSidebar">
      ...
      <NavLink to="/businesses/business-permit-license-office" name="Business Permit and Licensing Office" />
    </SubNavs>
      ...
      <div v-for="(services, key, idx) in serviceTypes" :key="idx" class="w-full">
        ...
        <div class="hidden child-sub-navs w-full">
          <div
            ...
            <NavLink v-for="(service, id) in services" :key="id" :name="service" :to="`/services/${key.toLowerCase().replace(/ /g, '-')}/${service
              .toLowerCase()
              .replace(/ /g, '-')}`" />
          </div>
        </div>
    ...
    <SubNavs v-show="isWholeSidebar">
      ...
      <NavLink to="/about/faq" name="FAQs" />
    </SubNavs>
  </nav>
</template>
