<template>
  <div class="fixed top-0 left-0 right-0 bottom-0 bg-black/20 flex items-center justify-center z-30">
    <form @submit.prevent="onSubmit"
      class="backdrop-blur-xl bg-white/70 h-full sm:h-auto sm:w-[1000px] p-5 rounded-lg flex flex-col sm:flex-row border overflow-y-auto scrollbar"
      ...
      <!-- news thumbnail -->
      <div
        class="overflow-hidden group relative h-full w-full flex-1 md:w-1/2 flex sm:flex-col items-start justify-between">
        <div
          class="overflow-hidden flex-1 h-full max-h-[300px] flex items-center justify-center w-full border-2 border-blue-600 rounded-md">
        ...
        <div
          class="flex-1 flex flex-wrap sm:flex-row items-start gap-2 overflow-y-auto sm:overflow-x-auto scrollbar py-2 w-full el-main h-[100px] sm:h-auto">
          ...
          <div>
            ...
            <button type="button" @click="selectNewImgFile"
              class="text-center flex items-center justify-center rounded-md px-2 border-blue-600 cursor-pointer border-2 w-[50px] sm:w-[80px] h-[50px] sm:h-[80px]">
              <i class="uil uil-image-plus text-sm sm:text-base pointer-events-none"></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 pl-5 w-full flex flex-col justify-start mt-3 md:mt-0 gap-3">
        <div>
          ...
          <textarea v-model="formData.title" @input="addHeight" rows="1"
            class="scrollbar focus:outline-blue-600 bg-transparent capitalize w-full text-xl font-bold max-h-[100px] border"></textarea>
          <!-- date -->
        ...
        <label class="flex items-center">
          <span class="text-xs text-gray-500 font-bold mr-1">By</span>
          <input v-model="formData.authors" @input="addHeight" rows="1"
            class="scrollbar focus:outline-blue-600 bg-transparent capitalize w-full text-sm font-bold border" />
        </label>

        <!-- location, if exists -->
        <div>
          <label v-if="selectedData.location" class="flex items-center">
            <i class="uil uil-map-marker-alt"></i>
            <input v-model="formData.location"
              class="ml-1 scrollbar focus:outline-blue-600 bg-transparent w-full text-sm border" />
          </label>

          <!-- category -->
          <label v-if="selectedData.category" class="flex items-center gap-1">
            <span class="font-[500] text-xs capitalize">category</span>
            <input :readonly="isApartment" v-model="formData.category"
              class="scrollbar focus:outline-blue-600 bg-transparent w-full text-sm border" />
          </label>
        </div>
        <!-- content -->
        <div class="overflow-y-scroll sm:overflow-y-auto sm:h-auto scrollbar">
          <span class="font-[500] text-xs capitalize mt-3">
            content
            <button ref="editBtn" type="button" @click="startEditing"
              class="text-blue-600 cursor-pointer hover:text-blue-500 self-start text-xs ml-2">
              edit
            </button>
          </span>
          <div>
        ...
        <div class="self-end flex items-center mb-0 mt-auto">
          <p v-if="selectedData.deleted_at" class="text-xs text-gray-400">
