<template>
  <div class="bg-slate-200/30 rounded-md p-5 flex flex-col w-full sm:w-[300px] h-[450px] overflow-hidden relative">
    <!--top  -->
    ...
    <DeleteVerificationModal v-if="isVerificationModal" :closeModal="showDeleteVerificiationModal" :id="data.id" :handleDelete="onDelete" />
    ...
    <div class="flex-1">
      ...
      <div class="flex items-center gap-2 mt-3 flex-wrap">
        <div v-for="jt in workTypes" class="px-2 py-1 text-[10px] rounded-full capitalize font-[500]" :style="{
    ...
    <div class="mt-8 overflow-hidden flex-1">
      <h5 class="font-bold text-gray-900">{{ new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(data.job_salary) }}</h5>
      <h6 class="text-gray-600 font-bold text-sm">{{ data.job_location }}</h6>
      <p class="text-xs text-gray-400 whitespace-pre-wrap w-full h-[50px] text-ellipsis truncate">{{ data.job_description
      }}</p>
    </div>
