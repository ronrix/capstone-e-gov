<template>
    <div class="fixed top-0 left-0 right-0 bottom-0 bg-black/20 flex items-center justify-center z-30">
        <form @submit.prevent="onSubmit"
            class="backdrop-blur-xl bg-white/70 sm:h-auto sm:w-[500px] p-5 rounded-lg flex flex-col border overflow-y-auto scrollbar"
            enctype="multipart/form-data">
            <!-- close modal btn -->
            <i @click="closeModal"
                class="uil uil-times text-black hover:text-blue-500 text-xl absolute top-0 right-2 cursor-pointer"></i>

            <h3 class="font-bold text-2xl">Create new FAQ</h3>
            ...
            <label class="flex flex-col">
                ...
                <a target="_blank" href="https://www.markdownguide.org/basic-syntax/"
                    ...
                    click here to learn how to format description
                </a>
...
function onSubmit() {
    ...
    handleCreateFAQ(formData).then(() => {
        isSubmitting.value = false;
    })
}

const { handleCreateFAQ } = defineProps({
    closeModal: Function,
    handleCreateFAQ: Function
})
</script>
