<template>
    ...
    <DisplayRequestModal v-if="isDisplayRequestModal" :close-modal="closeDisplayRequestModal" :selected-data="selectedData"
        :decline-request="declineRequest" :accept-request="acceptRequest" />
    ...
    <div class="container mx-auto flex mt-2 flex-col md:flex-row">
        ...
        <section class="bg-white flex-1 p-3 flex flex-col gap-10">
            <div>
                ...
                <h5 class="text-sm text-gray-600">Today is <span class="font-bold">{{ dateFormat(new Date()) }}</span></h5>
            </div>

            <div class="flex-1 flex flex-col justify-evenly bg-gradient-to-r from-sky-500 to-indigo-500 text-white px-3">
                <p class="text-slate-100 tracking-wide text-xs uppercase">qoute of the day</p>
                <h4 class="text-2xl font-[500]">"{{ qoute.text }}"</h4>
                <h5 class="text-sm text-slate-100"><i class="uil uil-angle-right"></i> {{ qoute.author }}</h5>
            </div>
            ...
            <div class="p-3 bg-gray-900 text-white">
                ...
                <div class="flex flex-wrap">
                    ...
                    <RouterLink to="/government/programs-anad-events"
                        class="border border-slate-100 px-3 py-1 hover:bg-slate-100 hover:text-gray-800">Programs &
                        Events</RouterLink>
                    <RouterLink to="/government/job-opportunities"
                        class="border border-slate-100 px-3 py-1 hover:bg-slate-100 hover:text-gray-800">Job
                        Opportunities
                    </RouterLink>
                    <RouterLink to="/government/full-disclosure-reports"
                        class="border border-slate-100 px-3 py-1 hover:bg-slate-100 hover:text-gray-800">Full disclosure
                        reports</RouterLink>
                    ...
                    <RouterLink to="/government/demographic-profile"
                        class="border border-slate-100 px-3 py-1 hover:bg-slate-100 hover:text-gray-800">Demographic
                        profile
                    </RouterLink>
                    <RouterLink to="/tourism/tourist-spots"
                        class="border border-slate-100 px-3 py-1 hover:bg-slate-100 hover:text-gray-800">Tourist
                    ...
                    <RouterLink to="/government/business-establishment"
                        class="border border-slate-100 px-3 py-1 hover:bg-slate-100 hover:text-gray-800">Business
                        establishments</RouterLink>
        ...
        <section class="flex-1 p-3">
            ...
            <div v-else class="overflow-y-scroll max-h-[500px] h-[500px] flex flex-col gap-3 scrollbar">
...
async function declineRequest(id) {
    await axios.get(be_url + '/post-request/decline/' + id)
        .then(({ data }) => {
...
function showDisplayRequestModal(data) {
    ...
    if (selectedData.value.status) {
        axios.get(be_url + "/notify-status/" + data.id)
            .then(({ data }) => {
                notifications.value = data.notifications.sort((a, b) => a.created_at < b.created_at);
            })
...
onMounted(() => {
    ...
    window.Echo.channel("notification")
        .listen("PostRequestNotification", (e) => {
    ...
    axios.get(be_url + "/notifications")
        .then(({ data }) => {
            notifications.value = data.notifications.sort((a, b) => a.created_at < b.created_at);
        })
        .catch(err => console.log(err));
