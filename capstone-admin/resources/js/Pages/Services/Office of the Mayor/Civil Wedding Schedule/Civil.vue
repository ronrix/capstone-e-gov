...
function handleUpdateRequirement(e, arrId) {
    axios.post(be_url + "/civil-wedding/requirement/edit", {
        rowId: dataCivil.value.id,
        arrId,
        newValue: e.target.value
    })
    .then(({data}) => {
       console.log(data.civil);
    })
...
async function handleDeleteRequirement(arrId) {
    return await axios.post(be_url + "/civil-wedding/requirement/delete", {
        rowId: dataCivil.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        dataCivil.value = data.civil[0];

        // get all the lists of requirements by looping through the data.mayor_clearance
        requirements.value = data.civil[0].service_requirements.split(",");
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateRequirement(newRequirement) {
    return await axios.post(be_url + "/civil-wedding/requirement/add", {
        rowId: dataCivil.value.id,
        newRequirement
    })
    .then(({data}) => {
        ...
        // get all the lists of requirements by looping through the data.civil
        requirements.value = data.civil[0].service_requirements.split(",");
        return data;
    })
    .catch(err =>  {
...
async function handleUpdateProcess(e, arrId, key) {
    return await axios.post(be_url + "/civil-wedding/process/edit", {
        rowId: dataCivil.value.id,
        arrId,
        newValue: e.target.value,
        key
...
async function handleDeleteProcess(arrId) {
    return await axios.post(be_url + "/civil-wedding/process/delete", {
        rowId: dataCivil.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        processes.value = JSON.parse(data.civil[0].service_process);
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateProcess(formData) {
    return await axios.post(be_url + "/civil-wedding/process/add", {
        rowId: dataCivil.value.id,
...
onMounted(() => {
    axios.get(be_url + "/civil-wedding")
    .then(({ data }) => {
