<template>
    <HeadTitle title="Government ID"></HeadTitle>
    <WrapperContent>
        <!-- response message -->
...
function handleUpdateRequirement(e, arrId) {
    axios.post(be_url + "/government-id/requirement/edit", {
        rowId: dataGovernmentId.value.id,
        arrId,
        newValue: e.target.value
    })
    .then(({data}) => {
       console.log(data.government_id);
    })
...
async function handleDeleteRequirement(arrId) {
    return await axios.post(be_url + "/government-id/requirement/delete", {
        rowId: dataGovernmentId.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        dataGovernmentId.value = data.government_id[0];

        // get all the lists of requirements by looping through the data.government_id
        requirements.value = data.government_id[0].service_requirements.split(",");
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateRequirement(newRequirement) {
    return await axios.post(be_url + "/government-id/requirement/add", {
        rowId: dataGovernmentId.value.id,
        newRequirement
    })
    .then(({data}) => {
...
async function handleUpdateProcess(e, arrId, key) {
    return await axios.post(be_url + "/government-id/process/edit", {
        rowId: dataGovernmentId.value.id,
        arrId,
        newValue: e.target.value,
        key
    ...
    .then(({data}) => {
       console.log(data.government_id[0]);
    })
    .catch(err => console.log(err));
}
...
async function handleDeleteProcess(arrId) {
    return await axios.post(be_url + "/government-id/process/delete", {
        rowId: dataGovernmentId.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        // get all the services processes by looping through the data.government_id
        processes.value = JSON.parse(data.government_id[0].service_process);
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateProcess(formData) {
    return await axios.post(be_url + "/government-id/process/add", {
        rowId: dataGovernmentId.value.id,
    ...
    .then(({data}) => {
        ...
        // get all the services processes by looping through the data.civil
        processes.value = JSON.parse(data.government_id[0].service_process);
        return data;
    })
    .catch(err =>  {
...
onMounted(() => {
    axios.get(be_url + "/government-id")
    .then(({ data }) => {
