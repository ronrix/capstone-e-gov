...
const dataTreatmentVaccination = ref([]);
const processes = ref([]);
...
function handleUpdateRequirement(e, arrId) {
    axios.post(be_url + "/treatment-vaccination-of-animals/requirement/edit", {
        rowId: dataTreatmentVaccination.value.id,
        arrId,
        newValue: e.target.value
    })
    .then(({data}) => {
       console.log(data.treatment_vaccination);
    })
...
async function handleDeleteRequirement(arrId) {
    return await axios.post(be_url + "/treatment-vaccination-of-animals/requirement/delete", {
        rowId: dataTreatmentVaccination.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        dataTreatmentVaccination.value = data.treatment_vaccination[0];

        // get all the lists of requirements by looping through the data.treatment_vaccination
        requirements.value = data.treatment_vaccination[0].service_requirements.split(",");
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateRequirement(newRequirement) {
    return await axios.post(be_url + "/treatment-vaccination-of-animals/requirement/add", {
        rowId: dataTreatmentVaccination.value.id,
        newRequirement
    })
    .then(({data}) => {
...
async function handleUpdateProcess(e, arrId, key) {
    return await axios.post(be_url + "/treatment-vaccination-of-animals/process/edit", {
        rowId: dataTreatmentVaccination.value.id,
        arrId,
        newValue: e.target.value,
        key
    ...
    .then(({data}) => {
       console.log(data.treatment_vaccination[0]);
    })
    .catch(err => console.log(err));
}
...
async function handleDeleteProcess(arrId) {
    return await axios.post(be_url + "/treatment-vaccination-of-animals/process/delete", {
        rowId: dataTreatmentVaccination.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        // get all the services processes by looping through the data.treatment_vaccination
        processes.value = JSON.parse(data.treatment_vaccination[0].service_process);
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateProcess(formData) {
    return await axios.post(be_url + "/treatment-vaccination-of-animals/process/add", {
        rowId: dataTreatmentVaccination.value.id,
...
onMounted(() => {
    axios.get(be_url + "/treatment-vaccination-of-animals")
    .then(({ data }) => {
