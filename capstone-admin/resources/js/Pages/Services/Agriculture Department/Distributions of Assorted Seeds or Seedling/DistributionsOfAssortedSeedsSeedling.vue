<template>
    <HeadTitle title="Distributions of Assorted Seeds"></HeadTitle>
    <WrapperContent>
        <!-- response message -->
...
const dataDistributionsAssortedSeeds = ref([]);
const processes = ref([]);
...
function handleUpdateRequirement(e, arrId) {
    axios.post(be_url + "/distributions-assorted-seeds/requirement/edit", {
        rowId: dataDistributionsAssortedSeeds.value.id,
        arrId,
        newValue: e.target.value
    })
    .then(({data}) => {
       console.log(data.distributions_seeds);
    })
...
async function handleDeleteRequirement(arrId) {
    return await axios.post(be_url + "/distributions-assorted-seeds/requirement/delete", {
        rowId: dataDistributionsAssortedSeeds.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        dataDistributionsAssortedSeeds.value = data.distributions_seeds[0];

        // get all the lists of requirements by looping through the data.distributions_seeds
        requirements.value = data.distributions_seeds[0].service_requirements.split(",");
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateRequirement(newRequirement) {
    return await axios.post(be_url + "/distributions-assorted-seeds/requirement/add", {
        rowId: dataDistributionsAssortedSeeds.value.id,
        newRequirement
    })
    .then(({data}) => {
...
async function handleUpdateProcess(e, arrId, key) {
    return await axios.post(be_url + "/distributions-assorted-seeds/process/edit", {
        rowId: dataDistributionsAssortedSeeds.value.id,
        arrId,
        newValue: e.target.value,
        key
    ...
    .then(({data}) => {
       console.log(data.distributions_seeds[0]);
    })
    .catch(err => console.log(err));
}
...
async function handleDeleteProcess(arrId) {
    return await axios.post(be_url + "/distributions-assorted-seeds/process/delete", {
        rowId: dataDistributionsAssortedSeeds.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        // get all the services processes by looping through the data.distributions_seeds
        processes.value = JSON.parse(data.distributions_seeds[0].service_process);
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateProcess(formData) {
    return await axios.post(be_url + "/distributions-assorted-seeds/process/add", {
        rowId: dataDistributionsAssortedSeeds.value.id,
...
onMounted(() => {
    axios.get(be_url + "/distributions-assorted-seeds")
    .then(({ data }) => {
