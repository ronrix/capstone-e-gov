...
function handleUpdateRequirement(e, arrId) {
    axios.post(be_url + "/spes/requirement/edit", {
        rowId: dataSPES.value.id,
        arrId,
        newValue: e.target.value
    })
    .then(({data}) => {
       console.log(data.spes);
    })
...
async function handleDeleteRequirement(arrId) {
    return await axios.post(be_url + "/spes/requirement/delete", {
        rowId: dataSPES.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        dataSPES.value = data.spes[0];

        // get all the lists of requirements by looping through the data.spes
        requirements.value = data.spes[0].service_requirements.split(",");
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateRequirement(newRequirement) {
    return await axios.post(be_url + "/spes/requirement/add", {
        rowId: dataSPES.value.id,
        newRequirement
    })
    .then(({data}) => {
...
async function handleUpdateProcess(e, arrId, key) {
    return await axios.post(be_url + "/spes/process/edit", {
        rowId: dataSPES.value.id,
        arrId,
        newValue: e.target.value,
        key
...
async function handleDeleteProcess(arrId) {
    return await axios.post(be_url + "/spes/process/delete", {
        rowId: dataSPES.value.id,
        arrId
    })
    .then(({data}) => {
        ...
        // get all the services processes by looping through the data.spes
        processes.value = JSON.parse(data.spes[0].service_process);
        return data;
    })
    .catch(err =>  {
        // set the response msg
...
async function handleCreateProcess(formData) {
    return await axios.post(be_url + "/spes/process/add", {
        rowId: dataSPES.value.id,
