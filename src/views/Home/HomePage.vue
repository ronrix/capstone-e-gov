...
import NewsAnnouncement from './NewsAnnouncement.vue'

const isShowNewsAnnouncement = ref(false)
...
// this function shows the news and announcements when this is invoked
...
<template>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-12 relative">
    <!-- news | announcements for bigger viewports -->
    <NewsAnnouncement
      :show="isShowNewsAnnouncement"
      :show-news-announcement="showNewsAnnouncement"
    />
    <WrapperContainer class="relative col-span-12 sm:col-span-8 md:col-span-10">
      <Events />
      <!-- assistance section -->
      ...
      <section class="mt-20 flex flex-col gap-5">
        <h4 class="font-['display'] text-5xl sm:w-3/4 text-center mx-auto dark:text-bggray">
          These are the <span class="bg-primary px-3 text-white">3</span> places you want to come
          and visit in Pililla
        </h4>

        <div
          class="relative mt-5 bg-gradient-to-l from-dark to-darkgray rounded-md text-white py-5"
        >
          <!-- map icon -->
          <div
